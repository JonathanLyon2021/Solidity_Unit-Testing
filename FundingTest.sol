...
contract FundingTest {
    ...
    function testAcceptingDonations() public {
       // Funding funding = new Funding();
        Assert.equal(funding.moneyRaised(), 0, 'Initial raised amount is different from 0');

        funding.donate{value: 10000000 gwei}();
        funding.donate{value: 20000000 gwei}();
        Assert.equal(funding.moneyRaised(), 30000000 gwei, 'Raised amount is different from sum of donations');
    ...
    function testSettingAnOwnerOfDeployedContract() public {
        ...
        Assert.equal(newFund.owner(), msg.sender, 'The owner is different from the deployer');
    }
}
